...
class UserLink(models.Model):
    type = models.ForeignKey(LinkType, verbose_name=("Link type"), on_delete=models.CASCADE, blank=True)
...
class User(AuthUser):
    ...
    links = geoModels.ForeignKey(UserLink, verbose_name=("Links"), on_delete=geoModels.CASCADE, blank=False)
...
class Event(geoModels.Model):
    ...
    category = geoModels.ForeignKey(Categories, verbose_name=("Category"), on_delete=geoModels.CASCADE)
    address = geoModels.CharField(max_length=255)
    location = geoModels.PointField(geography=True, blank=False, null=False)
     
    links = geoModels.ForeignKey(EventLink, verbose_name=("Links"), on_delete=geoModels.CASCADE, blank=False)
